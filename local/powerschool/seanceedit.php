...
$recordtoinsert = new seance();

    if($fromform->id) {

        $recordtoinsert->update_seance($fromform->id, $fromform->libelleseance,$fromform->idsalle,$fromform->idmatiere);
        redirect($CFG->wwwroot . '/local/powerschool/seance.php', 'Bien modifier');
...
if ($id) {
    ...
    $seance = $newseance->get_seance($id);
    if (!$seance) {
