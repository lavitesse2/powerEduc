...
class matiere extends moodleform {
     ...
     */
    public function update_matiere(int $id, string $libellematiere ): bool
    {
        global $DB;
        global $USER;
        $object = new stdClass();
        $object->id = $id;
        $object->numeromatiere = $libellematiere ;
        $object->usermodified = $USER->id;
        $object->timemodified = time();



        return $DB->update_record('matiere', $object);
    }
    ...
    public function get_matiere(int $matiereid)
    {
        global $DB;
        return $DB->get_record('matiere', ['id' => $matiereid]);
    }
     ...
     */
    public function supp_matiere(int $id)
    {
        global $DB;
        $transaction = $DB->start_delegated_transaction();
        $suppcampus = $DB->delete_records('matiere', ['id'=> $id]);
        if ($suppcampus){
