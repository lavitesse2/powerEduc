...
class seance extends moodleform {
    ...
    public function definition() {
        ...
        $salle = $req->select($sql1);
        $matiere = $req->select($sql2);
        ...
        // die;
        $mform->addElement('select', 'idsalle', 'Salle', $select1); // Add elements to your form
        $mform->setType('idsalle', PARAM_TEXT);                   //Set type of element
        $mform->setDefault('idsalle', '');        //Default value

        $mform->addElement('select', 'idmatiere', 'Matiere', $select2); // Add elements to your form
     ...
     */
    public function update_seance(int $id, string $libelleseance,int $idsalle,int $idmatiere ): bool
    {
        global $DB;
        global $USER;
        $object = new stdClass();
        $object->id = $id;
        $object->numeroseance = $libelleseance ;
